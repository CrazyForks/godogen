...
- Scenes live in `src/game/scenes/` and are addressed by `?scene=<name>` through `registry.ts`. Build in `main`; add a route only for on-demand isolation. A registry entry must point at a real module file or the route 404s.
...
- Hardware WebGL2 matters. The capture script warns but does not abort on software renderers — on a host with a GPU, a `[capture] WARNING` about SwiftShader/llvmpipe/lavapipe means the browser is misconfigured and the GPU path needs to be fixed so your screenshots match what the user sees live.
